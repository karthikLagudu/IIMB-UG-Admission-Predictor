    ...
    <form className="ug-candidate-form" onSubmit={props.onSubmit} noValidate>
      ...
      <fieldset>
        ...
        <div className="ug-field-grid">
          ...
          <label className={fieldIssue("class10OverallPercent") ? "has-error" : ""}><span>Class X overall %</span><input data-field="class10OverallPercent" aria-invalid={Boolean(fieldIssue("class10OverallPercent"))} type="number" min={IIMB_UG_2027_POLICY.eligibility.class10OverallPlanningMinimum} max="100" step="0.01" value={candidate.class10OverallPercent ?? ""} onChange={(event) => number("class10OverallPercent", event.target.value)} required /><small className="ug-field-hint">Enter {IIMB_UG_2027_POLICY.eligibility.class10OverallPlanningMinimum}% to 100%.</small>{fieldIssue("class10OverallPercent") && <small className="ug-field-error">Enter a Class X overall percentage from {IIMB_UG_2027_POLICY.eligibility.class10OverallPlanningMinimum}% to 100%.</small>}</label>
